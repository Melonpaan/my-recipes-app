...
import { translateDifficulty } from '../../../utils/translationUtils'
...
export function RecipesTable({ recipes, categories, onEdit, onDelete, onManageIngredients }: RecipesTableProps) {
  ...
  if (recipes.length === 0) {
    return <div className="mt-16 text-center opacity-70">Aucune recette pour le moment</div>
  }
      ...
      <table className="min-w-full text-sm">
        <thead className="bg-white/10">
          <tr>
            ...
            <th className="px-4 py-3 text-left">Ingrédients</th>
        ...
        <tbody className="divide-y divide-white/10">
          {recipes.map((r) => (
            <tr key={r.id}>
              ...
              <td className="px-4 py-3">{translateDifficulty(r.difficulty)}</td>
